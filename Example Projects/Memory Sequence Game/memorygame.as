...
/*	import flash.net.FileReference;
    import flash.net.FileFilter;*///Required for URL XML import
	...
	//GAME FUNCTIONS -----------------------------
		...
		private function loadHighscore(){
			...
			/*var xmlURL:URLRequest = new URLRequest("Highscore.xml");
			var xmlLoader:URLLoader = new URLLoader(xmlURL);
			xmlLoader.addEventListener(Event.COMPLETE,xmlLoaded);*/

			//Reading from LOCAL file
			var file:File = File.desktopDirectory.resolvePath("MemoryGame/Highscore.xml");//Gte the path for the file
			//var file:File = File.documentsDirectory.resolvePath("MemoryGame/Highscore.xml");//CAN'T SAVE ON NETWORK DRIVE IN AIR 1.5
			if (file.exists == false){//Check if the highscore file exists, else make a blank new one.
				makeStorageFile();
			}
			var fileStream:FileStream = new FileStream();//Create the filestream
			fileStream.open(file, FileMode.READ);//Open the filestrem
			xmlHighscore = XML(fileStream.readUTFBytes(fileStream.bytesAvailable));//transfer the data into a variable
		...
		private function makeStorageFile(){
			...
			var file:File = File.desktopDirectory.resolvePath("MemoryGame/Highscore.xml");
			//var file:File = File.documentsDirectory.resolvePath("MemoryGame/Highscore.xml");//CAN'T SAVE ON NETWORK DRIVE IN AIR 1.5
		...
		private function saveHighscore():void {
			...
			file = file.resolvePath("MemoryGame/Highscore.xml");
			fileStream.open(file, FileMode.WRITE);
		...
		private function goToEndOfGame (){
				MovieClip(root).rank1 = xmlHighscore.Rank1.@score;//Pass those values to the variables in the FLA file
				...
				MovieClip(root).gotoAndStop("GameOver");//Go to the game over frame in the FLA file
